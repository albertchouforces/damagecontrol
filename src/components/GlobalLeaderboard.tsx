...
import { QUIZ_CONFIG, SECOND_QUIZ_CONFIG } from '../data/templateQuiz';
...
export function GlobalLeaderboard({ onClose }: GlobalLeaderboardProps) {
  ...
  const [selectedQuiz, setSelectedQuiz] = useState<'quiz1' | 'quiz2'>('quiz1');
  ...
  useEffect(() => {
    const fetchScores = async () => {
      ...
      setError(null);
      try {
        const quizName = selectedQuiz === 'quiz2' ? SECOND_QUIZ_CONFIG.quiz_name : QUIZ_CONFIG.quiz_name;
        ...
        <div className="flex-1 overflow-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <Loader2 className={`animate-spin text-${currentQuizConfig.themeColor}-500`} size={32} />
            </div>
          ) : error ? (
